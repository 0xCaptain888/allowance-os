...
| Reliability | 7.5/10 | 64 source/unit tests, independent AlphaBrief verifier, configurable mobile RPC with bounded retries, HTTP-only deployment confirmation, safe runner resume, and an incident runbook | Instrumentation/E2E tests, multi-endpoint production failover, offline UX and operational monitoring |
...
AlphaBrief now provides the strongest product proof: real delivery, independent verification, authority-free v2 settlement, and bad-output freeze. The remaining four catalog entries are correctly treated as templates rather than claimed live integrations.
